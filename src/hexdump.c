...
typedef size_t usize; // unsigned int di grandezza pari a arch del processore
typedef uint8_t u8; // unsigned int 8 bit
typedef uint32_t u32; //unsigned int 32 bit
...
void hexdump(void *buffer, usize size) {
  ...
  for (i = 0; i < size; i++){
    ...
    // after the first 8 bytes, print a whitespace
    if ( (i % 8) == 0) {
      printf(" ");
    }
    ...
    if ( ((i % 16) == 15) || (i == size - 1) ) {
      // TODO: add extra whitespaces if needed
      for (j = 0; j < 15 - (i % 16); j++) {
	printf("   ");
      }
      printf(" |");
